...
class Program {
    ...
    public Program addMethodToClass(int node, String methodName) {
        ...
        for (int i = 0; i < compilationUnit.getChildNodes().size(); i++) {
            ...
            compilationUnit.getChildNodes().get(i).toString().lines().forEach(l -> System.out.println(""));
        }
    ...
    public String run() {
        ...
        try {
            runProcess("pwd");
            System.out.println("**********");
            runProcess("javac -classpath javaparser-symbol-solver-core-3.6.23.jar:javaparser-core-3.6.23.jar src/no/iterate/ProgramTests.java src/no/iterate/Program.java src/no/iterate/Tester.java src/no/iterate/Reporter.java src/no/iterate/Testable.java src/no/iterate/TestResults.java src/no/iterate/Tests.java src/no/iterate/FizzBuzz.java -d out/production/cc18/ && \\\n");
            System.out.println("**********");
